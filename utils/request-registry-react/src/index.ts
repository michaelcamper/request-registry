...
> = TEndpointGetFunction extends EndpointGetFunction<infer TKeys, any> ? TKeys : never;
type EndpointResult<
  TEndpointGetFunction
...
export function useGetEndPoint<
  TEndpoint extends EndpointGetFunction<{ [key: string]: never | undefined }, any>
...
export function useGetEndPoint<TEndpoint extends EndpointGetFunction<any, any>>(
  ...
  const [endpointBecomesEnabledPromise, endpointEnabledState] = useWaitForValuePromise(
    isEndpointEnabled,
    (enabled) => enabled
  );
  ...
  const executeEndpoint = useCallback(() => {
    ...
    const currendEndpointPromise = endpointEnabledState.done
      ...
      : endpointBecomesEnabledPromise.then(() => endpoint(latestKeys.current));
  ...
  const [endpointState, updateEndpointState] = useEndpointStateReduce<TEndpoint>(
    executeEndpoint
  );
...
export function useGetEndPointLazy<
  TEndpoint extends EndpointGetFunction<{ [key: string]: never | undefined }, any>
>(endpoint: TEndpoint): EndpointState<EndpointResult<TEndpoint>>;
export function useGetEndPointLazy<TEndpoint extends EndpointGetFunction<any, any>>(
  endpoint: TEndpoint,
  keys: EndpointKeys<TEndpoint>,
  executeAjax?: boolean
): EndpointState<EndpointResult<TEndpoint>>;
export function useGetEndPointLazy<TEndpoint extends EndpointGetFunction<any, any>>(
  ...
  return typeof window === 'undefined'
    ? serverSideState
    : useGetEndPoint(endpoint, keys as EndpointKeys<TEndpoint>, executeAjax);
}
...
function useEndpointStateReduce<TEndpoint extends EndpointGetFunction<any, any>>(
  initialLoad: () => Promise<EndpointResult<TEndpoint>>
) {
  return useReducer(endpointStateReducer, initialLoad, initialEndpointState) as [
    EndpointState<EndpointResult<TEndpoint>>,
    (action: EndpointActions<TEndpoint>) => EndpointState<EndpointResult<TEndpoint>>
  ];
}
