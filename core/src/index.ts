...
// GET Result
export interface EndpointDeleteFunction<TKeys extends {}, TResult extends unknown>
    ...
    Object.assign(
      {
        cacheKey: (keys: TKeys, baseKey: string) =>
          JSON.stringify(graphQLVariables ? graphQLVariables(keys) : keys) + baseKey,
      },
      options,
      {
        loader: (keys: TKeys, url: string, headers: { [key: string]: string }) =>
          // Build up the post body and execute the loader
          loader(keys, url, headers, {
            query: options.query,
            variables: graphQLVariables ? graphQLVariables(keys) : keys,
          }),
      }
    )
  );
